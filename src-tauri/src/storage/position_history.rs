use super::{db_error, decimal_value, Storage, WealthHistoryPoint};
...
fn read_positions(
    ...
    let mut statement = connection.prepare("SELECT p.id,p.label,p.holding_end_date FROM portfolio_positions p JOIN accounts a ON a.id=p.account_id WHERE a.id=?1 AND a.account_type='manual_asset' ORDER BY p.label,p.id")?;
...
mod tests {
    ...
    fn histories_keep_sold_positions_and_original_unit_prices() {
        ...
        super::super::initialize_schema(&connection).unwrap();
