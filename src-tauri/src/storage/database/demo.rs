//! Erzeugt ein synthetisches Finanzprofil für die Demoansicht.
...
pub(crate) fn read_demo_status(storage: &Storage) -> Result<bool, String> {
    let db = storage.connect().map_err(|_| LOCKED)?;
    Ok(is_demo(&db))
}

impl Storage {
    pub(crate) fn create_demo(&self, today: NaiveDate) -> Result<(), String> {
...
fn seed(db: &mut Connection, today: NaiveDate) -> rusqlite::Result<()> {
    ...
    while day <= today {
        ...
        if day.day() == 28 && card_balance < 0 {
            ...
            transaction.execute("INSERT INTO transaction_reporting_flags(transaction_id,exclude_from_cashflow,is_settlement) VALUES(?1,1,1)", [transaction.last_insert_rowid()])?;
            booking(
                &transaction,
                4,
                import_card,
...
mod tests {
    ...
    fn demo_history_reconciles_balances_holdings_and_tax_years() {
        ...
        assert!(is_demo(&db));
        crate::storage::consumption::prepare(&db).unwrap();
        assert_eq!(
            db.query_row(
                "SELECT COUNT(*) FROM transaction_reporting_flags WHERE is_settlement=1",
                [],
                |row| { row.get::<_, i64>(0) }
            )
            .unwrap(),
        ...
        assert_eq!(db.query_row("SELECT COUNT(*) FROM transactions t JOIN transaction_reporting_flags s ON s.transaction_id=t.id AND s.is_settlement=1 WHERE t.account_id=4 AND t.amount_minor<0", [], |row| row.get::<_, i64>(0)).unwrap(), 0);
        assert_eq!(
    ...
    fn creating_demo_preserves_existing_database_and_supports_reopening() {
        ...
        assert!(is_reusable_demo(
            &directory.path().join("Demo 1.vault.sqlite3"),
            "Demo 1"
        ));
