...
pub(super) fn read_settings(db: &Connection) -> Result<AppSettings, String> {
...
fn open(path: &Path, password: &str, create: bool) -> rusqlite::Result<Connection> {
...
impl Storage {
    ...
    pub(super) fn connect(&self) -> rusqlite::Result<VaultConnection<'_>> {
        let lease = self
            .session
            .read()
    ...
    fn unlock(&self, password: String, setup: bool) -> Result<(), String> {
    ...
    fn create_vault(&self, password: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(test)]
    pub(super) fn test_storage(path: std::path::PathBuf) -> Self {
...
mod tests {
    use super::*;
    fn locked(directory: &Path) -> Storage {
        Storage {
    ...
    #[test]
    fn fixed_factor_anonymization_preserves_amount_ratios_and_balances() {
        let dir = tempfile::tempdir().unwrap();
        let vault = locked(dir.path());
        vault.unlock(PASSWORD.into(), true).unwrap();
        vault.connect().unwrap().execute_batch(
    ...
    #[test]
    fn settings_persist_and_control_backend_expiry() {
        let dir = tempfile::tempdir().unwrap();
        let vault = locked(dir.path());
    ...
    #[test]
    fn password_change_requires_current_password_and_preserves_data() {
        let dir = tempfile::tempdir().unwrap();
        let vault = locked(dir.path());
        assert!(vault
    ...
    #[test]
    fn encrypted_at_rest_restart_wrong_password_and_manual_lock() {
        let dir = tempfile::tempdir().unwrap();
        let vault = locked(dir.path());
        assert!(vault.connect().is_err());
    ...
    #[test]
    fn inactivity_expires_in_backend_without_frontend_and_cannot_be_revived() {
        let dir = tempfile::tempdir().unwrap();
        let vault = locked(dir.path());
        vault.unlock(PASSWORD.into(), true).unwrap();
    ...
    #[test]
    fn setup_accepts_seven_characters_and_never_overwrites_existing_vault() {
        let dir = tempfile::tempdir().unwrap();
        let vault = locked(dir.path());
