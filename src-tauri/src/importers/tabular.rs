...
fn detect_header_row(rows: &[&[Data]]) -> Option<(usize, usize)> {
    ...
    for (index, row) in rows.iter().take(PREVIEW_ROWS).enumerate() {
        ...
        if score >= 3 && best.map(|(_, best_score)| score > best_score).unwrap_or(true) {
...
fn header_score(row: &[Data]) -> usize {
    ...
    let has_date = find_header_optional(
        &headings,
        &["datum", "buchungsdatum", "buchungstag", "kaufdatum", "transaktionsdatum"],
    )
    .is_some();
    ...
    let has_amount = find_header_optional(
        &headings,
        &["betrag", "belastung", "gutschrift", "abfluss", "zufluss", "soll", "haben"],
    )
    .is_some();
    text_cells + usize::from(has_date) * 6 + usize::from(has_description) * 4 + usize::from(has_amount) * 3
}

pub(super) fn parse_mapped_workbook(
    path: &Path,
