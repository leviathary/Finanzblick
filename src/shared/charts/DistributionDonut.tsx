...
export type DonutSegment = { key: string; label: string; color: string; amountMinor: number; keys: string[] };
export function DistributionDonut({ segments, selected = [], onSelect, total, label, currency }: {
  ...
  const percent = (value:number) => new Intl.NumberFormat(locale(),{style:"percent",maximumFractionDigits:1}).format(value/sum);
  let offset = 0;
    ...
    <svg viewBox="0 0 240 240" aria-label={label}>
      ...
      {positive.map(item=>{
        ...
        return <circle key={item.key} cx="120" cy="120" r="94" pathLength="100" fill="none" stroke={item.color}
          ...
          role={onSelect ? "button" : "img"} tabIndex={0} aria-pressed={onSelect ? pressed : undefined} aria-label={description}
