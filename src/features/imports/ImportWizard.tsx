...
export function ImportWizard({ enabled = true }: { enabled?: boolean }) {
  ...
  return <section className="import-wizard" aria-labelledby="import-title" aria-busy={saving}>
    <div className="wizard-heading"><div><p className="eyebrow">{t("Import")}</p><h1 id="import-title">{t("Finanzdateien importieren")}</h1></div><span className="privacy-chip">{t("✓ Lokale Verarbeitung")}</span><p className="intro">{t("Bankauszüge und Depotbestände gemeinsam auswählen oder hierher ziehen. Der Dokumenttyp wird automatisch erkannt.")}</p></div>
    <div className={`drop-zone batch-drop${dragging ? " dragging" : ""}${busy ? " disabled" : ""}`} onClick={() => { if (!busy) void choose(false); }} onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); if (!isTauri()) setError(t("Drag-and-drop ist in der Desktop-App verfügbar.")); }}>
      ...
      <h2>{dragging ? t("Loslassen, um Dateien oder Ordner hinzuzufügen") : t("Dateien oder Ordner hierher ziehen")}</h2>
      <span className="batch-drop-separator">{t("oder")}</span>
      <button className="primary-button" disabled={busy} onClick={event => { event.stopPropagation(); void choose(false); }}>{t("Dateien auswählen")}</button>
      <small>{t("Bankauszüge und Depotbestände: XLSX, XLS, CSV, PDF, MT940 sowie camt.053 und camt.054 (ISO 20022 XML)")}</small>
    </div>
    <div className="batch-drop-options"><button className="secondary-button" disabled={busy} onClick={() => void choose(true)}>{t("Ordner auswählen")}</button><label className="batch-check"><input type="checkbox" checked={recursive} disabled={busy} onChange={event => setRecursive(event.target.checked)} />  {t("Unterordner einbeziehen")}</label></div>
    ...
    {items.length > 0 && <section className="dashboard-card import-batch-card" aria-label={t("Deine Importliste")}>
      ...
      <div className="history-table batch-table"><table><thead><tr><th>{t("Datei")}</th><th>{t("Anbieter")}</th><th>{t("Status")}</th><th>{t("Aktion")}</th></tr></thead><tbody>{displayedItems.map(item => <Fragment key={item.file.path}><tr className={active === item.file.path ? "batch-active" : ""}>
        <td><strong>{item.file.name}</strong><small>{formatFileSize(item.file.size)}{item.parsed ? tr` · ${item.parsed.transactions.length} Buchungen` : item.positionSnapshot ? tr` · ${item.positionSnapshot.positions.length} Positionen` : ""}</small></td>
