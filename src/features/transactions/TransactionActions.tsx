// Zeigt kompakte, tastaturbedienbare Aktionen für neutrale Buchungen ausserhalb der scrollenden Tabelle.
...
export function TransactionActions({ neutral, description, disabled, onChange }: {
  ...
  return <>
    ...
    {position && createPortal(<div className="transaction-action-menu" role="dialog" aria-label={t("Buchungsaktionen")} ref={menu}
      ...
      {neutral ? <button type="button" onClick={() => choose("NONE")}>{t("Als reguläre Buchung wiederherstellen")}</button> : <>
        <strong>{t("Als Umbuchung / Kartenausgleich markieren")}</strong>
        <button type="button" onClick={() => choose("CREDIT_CARD_SETTLEMENT")}>{t("Kartenausgleich")}</button>
        <button type="button" onClick={() => choose("INTERNAL_TRANSFER")}>{t("Übertrag zwischen eigenen Konten")}</button>
      </>}
      <a href="#transactions/transfers">{t("Umbuchungen & Ausgleiche")}</a>
    </div>, document.body)}
  </>;
}
