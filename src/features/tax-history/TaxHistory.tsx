...
import {
  ...
  formatDate,
  money,
  netValuesAreConsistent,
  previewValues,
  signedMoney,
  taxValueStatus,
  toEditable,
} from "./taxHistorySupport";

export interface TaxStatementPreview {
  sourceName: string;
  taxYear: number;
...
export interface TaxSnapshot extends Omit<TaxStatementPreview, "warnings" | "existingYear"> {
  id: number;
  importedAt: string;
}

export interface EditableTaxValues {
  grossAssets: string;
  liabilities: string;
...
export interface EditablePreview extends TaxStatementPreview, EditableTaxValues {
  sourcePath: string;
}

export interface ManualTaxEntry extends EditableTaxValues {
  taxYear: string;
}
...
export interface EditableSnapshot extends EditableTaxValues {
  id: number;
  taxYear: number;
  cantonTaxableWealthMinor: number | null;
}
...
export function TaxHistory({ view = "history", active = true }: { view?: TaxHistoryView; active?: boolean }) {
  ...
  const manualIncomplete = [manual.grossAssets, manual.liabilities, manual.taxableWealth, manual.securitiesAndCash, manual.realEstate, manual.otherAssets].some((value) => value.trim() === "");
  ...
  return (
    <section className="tax-history-page">
      ...
      {view === "management" && manualOpen && <form className="tax-preview-section tax-manual-entry" aria-labelledby="tax-manual-title" onSubmit={(event) => { event.preventDefault(); void saveManual(); }}>
        ...
        <div className={`tax-validation ${manualIncomplete ? "pending" : manualStatus}`} role="status">
          {manualIncomplete
            ? t("Fülle die leeren Betragsfelder aus. Danach wird die Berechnung geprüft.")
            : manualStatus === "valid"
            ? t("Plausibel: Vermögensaufteilung und Nettowert sind rechnerisch konsistent.")
            : manualStatus === "notice"
              ? t("Hinweis: Die Vermögensaufteilung entspricht nicht den gesamten Vermögenswerten. Das Steuerjahr kann trotzdem gespeichert werden.")
